							...
							<label for="customerFrom">客户来源</label> <select
								...
								<c:forEach items="${fromType}" var="item">
									<option value="${item.dict_id}"
										selected="<c:if test="${item.dict_id == custSource}"> selected</c:if> ">				 
										${item.dict_item_name }
									</option>
								</c:forEach>
							</select>
						...
						<div class="form-group">
							<label for="custIndustry">所属行业</label> <select
								...
								<c:forEach items="${industryType}" var="item">
									<option value="${item.dict_id}"
										selected="<c:if test="${item.dict_id == custIndustry}"> selected</c:if>">${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
						<div class="form-group">
							<label for="custLevel">客户级别</label> <select class="form-control"
								...
								<c:forEach items="${levelType}" var="item">
									<option value="${item.dict_id}"
										selected="<c:if test="${item.dict_id == custLevel}"> selected</c:if>">${item.dict_item_name }</option>
								</c:forEach>
							</select>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="edit_customerName"
									placeholder="客户名称" name="cust_name"></input>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-10">
								<select class="form-control" id="edit_customerFrom"
									...
									<c:forEach items="${fromType}" var="item">
										<option value="${item.dict_id}"
											selected="<c:if test="${item.dict_id == custSource}"> selected</c:if>">${item.dict_item_name }</option>
									</c:forEach>
								</select>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<select	class="form-control" id="edit_custLevel" name="cust_level">
									<option value="">--请选择--</option>
									<c:forEach items="${levelType}" var="item">
										<option value="${item.dict_id}" selected="<c:if test="${item.dict_id == custLevel}"> selected</c:if>" >${item.dict_item_name }</option>
									</c:forEach>
								</select>
							</div>
						</div>
